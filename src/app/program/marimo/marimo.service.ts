...
export class MarimoService {
            ...
            tap((res: any) => {
                ...
                if (Array.isArray(notebooks)) {
                    this.notebooks = notebooks
                        ...
                        .map(
                            (nb: MarimoNotebook) =>
                                new MarimoWorkbook(nb.title, nb.name),
                        );
    ...
    createNotebook(name: string, content?: string): Observable<any> {
        return this.apiService
            .post(`${this.baseUrl}/notebooks`, {name, content})
            .pipe(tap(() => this.getNotebooks().subscribe()));
    }

    renameNotebook(oldName: string, newName: string): Observable<any> {
        return this.apiService
            .post(`${this.baseUrl}/notebooks/${oldName}/rename`, {newName})
            .pipe(tap(() => this.getNotebooks().subscribe()));
    }

    deleteNotebook(name: string): Observable<any> {
        return this.apiService
            .delete(`${this.baseUrl}/notebooks/${name}`)
            .pipe(tap(() => this.getNotebooks().subscribe()));
    }
}
