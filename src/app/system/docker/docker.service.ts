...
export class DockerService {
  ...
  startContainer(name: string): Observable<ContainerActionResponse> {
    return this.apiService.post(`${UrlConstants.DOCKER}/containers/${name}/start`, {});
  }

  stopContainer(name: string): Observable<ContainerActionResponse> {
    return this.apiService.post(`${UrlConstants.DOCKER}/containers/${name}/stop`, {});
  }

  restartContainer(name: string): Observable<ContainerActionResponse> {
    return this.apiService.post(`${UrlConstants.DOCKER}/containers/${name}/restart`, {});
  }

  getContainerLogs(name: string, tail: number = 500): Observable<ContainerLogsResponse> {
    return this.apiService.get(`${UrlConstants.DOCKER}/containers/${name}/logs?tail=${tail}`);
  }

  clearContainerLogs(name: string): Observable<ContainerActionResponse> {
    return this.apiService.post(`${UrlConstants.DOCKER}/containers/${name}/clear-logs`, {});
  }
