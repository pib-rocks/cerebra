...
> A **draft** release does **not** create a git tag. Only _publishing_ it does.
...
_Resolve release tag from the merge_ step logs which tag it found.
...
the _nearest reachable_ tag, so on a merge that carries **no** new release it returns the
...
The failure mode matters more than the happy path: `git describe` fails _open_ (wrong
image published quietly), `git tag --points-at` fails _closed_ (loud error, nothing
pushed).
