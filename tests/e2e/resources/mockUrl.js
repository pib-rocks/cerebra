...
function _checkProperties(requiredProps, obj) {
    for (const prop of requiredProps) {
        if (Object.hasOwn(obj, prop) == false) {
            throw Error(
                `Provided argument object doesnt have required property ${prop}`,
            );
        }
    }
}
...
async function blockUrl(args, page, logger) {
    for (const arg of args) {
        ...
        if (
            _errorCodes.includes(arg.errorCode) ||
            arg.errorCode === undefined ||
            arg.errorCode === null
        ) {
            await page.route(String(arg.url), (route) => {
...
async function mockBrickletUidPutFinalFails(args, page, logger) {
    await page.route("**/api/bricklet/*", (route) => {
        ...
        try {
            ...
            uid = body ? JSON.parse(body).uid || "" : "";
        } catch (error) {
            uid = "";
        }
        if (String(uid).startsWith("temp")) {
            route.fulfill({
                status: 200,
...
mockBrickletUidPutFinalFails.rfdoc = `
 Keyword mocks bricklet PUT so temp-uid pass succeeds and final pass returns HTTP 500.
`;
...
exports.mockBrickletUidPutFinalFails = mockBrickletUidPutFinalFails;
