...
describe("DiagnosticsService hardware-config", () => {
    ...
    it("should POST hardware-config import via ApiService", () => {
        let result: HardwareConfig | undefined;
        service
            .importHardwareConfig(sampleConfig)
            .subscribe((config) => (result = config));
    ...
    it("should reject duplicate UIDs and invalid UID formats", () => {
        ...
        expect(result.valid).toBeFalse();
        expect(
            result.errors.some((e) => e.includes("Duplicate Bricklet UID")),
        ).toBeTrue();
        expect(
        ...
        const exportedFile = JSON.stringify(
            {
                ...
                motors: [
                    ...
                    },
                    {
                        ...
                        invert: false,
                        brickletPins: [
                            {brickletNumber: 1, pin: 8, invert: false},
                        ],
                    },
                ],
            },
    ...
    it("should reject missing bricklets/motors arrays", () => {
        ...
        expect(result.valid).toBeFalse();
        expect(
            result.errors.some((e) => e.includes("'bricklets' array")),
        ).toBeTrue();
        expect(
            result.errors.some((e) => e.includes("'motors' array")),
        ).toBeTrue();
    });
});
