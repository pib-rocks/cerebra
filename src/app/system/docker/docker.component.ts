...
export class DockerManagementComponent implements OnInit, OnDestroy {
  ...
  startContainer(name: string): void {
    ...
    this.dockerService.startContainer(name).subscribe({
      next: (res) => {
        ...
        this.successMessage = res.message || `Container ${name} started.`;
        this.refreshContainers();
      },
      error: (err) => {
        this.actionLoading[name] = false;
  ...
  stopContainer(name: string): void {
    ...
    this.dockerService.stopContainer(name).subscribe({
      next: (res) => {
        this.actionLoading[name] = false;
        this.successMessage = res.message || `Container ${name} stopped.`;
        this.refreshContainers();
      },
      error: (err) => {
        this.actionLoading[name] = false;
  ...
  restartContainer(name: string): void {
    ...
    this.dockerService.restartContainer(name).subscribe({
      next: (res) => {
        this.actionLoading[name] = false;
        this.successMessage = res.message || `Container ${name} restarted.`;
        this.refreshContainers();
      },
      error: (err) => {
        this.actionLoading[name] = false;
  ...
  purgeDocker(): void {
    if (!confirm("Are you sure you want to purge unused Docker resources (system prune)?")) {
      return;
    }
    ...
    this.dockerService.purgeDocker().subscribe({
      next: (res) => {
        this.purging = false;
        this.successMessage = res.message || "Docker purge completed successfully.";
        this.refreshContainers();
      },
      error: (err) => {
        this.purging = false;
  ...
  fetchLogs(): void {
    ...
    this.dockerService.getContainerLogs(this.activeContainerName).subscribe({
      next: (res) => {
  ...
  clearLogs(): void {
    ...
    this.dockerService.clearContainerLogs(this.activeContainerName).subscribe({
      next: () => {
        this.logsText = "";
        this.cdr.markForCheck();
      },
  ...
  private scrollToBottom(): void {
    setTimeout(() => {
      if (this.logContainerElement && this.logContainerElement.nativeElement) {
  ...
  getStatusBadgeClass(status: string | undefined): string {
    ...
    if (st === "exited" || st === "stopped" || st === "unhealthy") return "badge-danger";
